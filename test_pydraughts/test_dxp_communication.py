...
def test_dam_exchange():
    ...
    assert (dam_exchange.parse('ANAME                            1') ==
            {'type': 'A', 'engineName': 'NAME', 'accCode': '1'})  # GAMEACC
    assert (dam_exchange.parse('M000135240130') ==
            {'type': 'M', 'time': '0001', 'from': '35', 'to': '24', 'nCaptured': '01', 'captures': ['30']})  # MOVE
    assert (dam_exchange.parse(f'R01NAME                            Z010100BW{Board()._game.get_dxp_fen()}') ==
            {'type': 'R', 'name': 'NAME', 'fColor': 'Z', 'gameTime': '010', 'numMoves': '100', 'posInd': 'B',
             'mColor': 'W', 'pos': 'zzzzzzzzzzzzzzzzzzzzeeeeeeeeeewwwwwwwwwwwwwwwwwwww'})  # GAMEREQ

    assert (dam_exchange.parse(dam_exchange.msg_gamereq(0, 100, 100, Board(), 0)) ==
            {'type': 'R', 'name': 'DXP Client', 'fColor': 'Z', 'gameTime': '100', 'numMoves': '100', 'posInd': 'B',
             'mColor': 'W', 'pos': 'zzzzzzzzzzzzzzzzzzzzeeeeeeeeeewwwwwwwwwwwwwwwwwwww'})  # GAMEREQ
